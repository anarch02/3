        ...
        <div class="col-lg-8">
          <div class="card">
            <div class="card-body">
              ...
              <form class="row g-3 needs-validation" action="{{route($info['route'], $object->id)}}" novalidate>
                @foreach ($info['inputs'] as $key => $item)
                <div class="position-relative">
                  <label for="validationTooltip01" class="form-label">{{$item['label']}}</label>
                  <input type="{{$item['type']}}" class="form-control" id="validationTooltip01" name="{{$info['name']}}" value="{{$object[$item['name']]}}" required>
